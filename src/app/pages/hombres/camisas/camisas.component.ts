...
export class CamisasComponent implements OnInit, OnDestroy {
  ...
  addToCart(selectedProduct: any): void {
    ...
    this.toastModal(`${this.selectedProduct.name} agregado al carrito `, 'success');
    this.closeModal();
  }
  ...
  addToFavorites(): void {
    ...
    if (!this.selectedSize || !this.selectedVarianteId) {
      this.closeModal(); // ✅ Cerrar modal si no hay talla
  ...
  /** 🌑 Toast oscuro, centrado encima del modal */
